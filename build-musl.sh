...
build_arch() {
    ...
    elif [[ "$arch_key" == "sparc" || "$arch_key" == "sparc64" || "$arch_key" == powerpc || "$arch_key" == "powerpc-sf" || "$arch_key" == powerpcle || "$arch_key" == "powerpcle-sf" || "$arch_key" == "alphaev56" || "$arch_key" == "alphaev67" || "$arch_key" == "alpha" ]]; then
        arch_cflags="$CFLAGS -mlong-double-64"
        arch_cxxflags="$CXXFLAGS -mlong-double-64"
